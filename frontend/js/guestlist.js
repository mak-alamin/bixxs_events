function ticket_master_delete_guest(guest){
    if (ticket_master_count_guests() < 2){
...
function ticket_master_add_guest(){
    ...
    let actual_guest_count = ticket_master_count_guests();
    ...
    if ( actual_guest_count >= available_tickets){
        alert("Nicht genügend Tickets verfügbar. Bitte wählen Sie einen anderen Tag aus.");
        return;
    }
    ...
    new_guest.innerHTML = new_guest.innerHTML.replace(re, '$1' + ticket_master_get_next_guest_number());

    // Open details
    new_guest.innerHTML = new_guest.innerHTML.replace(/<details(.+)?>/g, '<details open>');

    // Remove values
    new_guest.innerHTML = new_guest.innerHTML.replace(/(<input.+)value="(.+?)"/g, '$1value=""');
...
function ticket_master_count_guests(){
    let guest_count = document.querySelectorAll('#mlx_guest_selection>div').length;
