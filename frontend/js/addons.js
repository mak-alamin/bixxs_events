...
function bixxs_events_calculate_sumary() {
  ...
  let fields = document.querySelectorAll(
    ".bixxs_events_addons_wrapper > input[type=text], .bixxs_events_addons_wrapper > textarea, .bixxs_events_addons_wrapper > input[type=number]"
  );

  for (let field of fields) {
    if (field.value === "") continue;

    if (field.type === "number" && field.value === "0") continue;
    ...
    if (field.dataset.pprice !== "0") {
      ...
      if (amount) {
        addon_wrapper.innerHTML +=
          "<div>" + amount + " x " + label + ": " + price + "</div>";
      } else {
        addon_wrapper.innerHTML +=
          "<div>" + guest_count + " x " + label + ": " + price + "</div>";
      }
    }
  }
  ...
  fields = document.querySelectorAll(
    ".bixxs_events_addons_wrapper > input[type=checkbox]"
  );
