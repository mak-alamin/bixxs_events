<?php
if(!class_exists("C0C7D3365")) {
	class C0C7D3365 {
    	public $key = "0C3511B32EE50339";
    	private $product_id = "11";
    	private $product_base = "?page=bixxs-events";
    	private $server_host = "https://bixxs.de/wp-json/bixxs_lizenzen/";
		...
		function __construct($plugin_base_file='')
		{
			...
			if($this->hasCheckUpdate) {
				if(function_exists("add_action")){
					add_action( 'admin_post_?page=bixxs-events_fupc', function(){
					...
					if($this->isTheme){
						...
						add_filter( 'plugin_row_meta', function($links, $plugin_file ){
							if ( $plugin_file == plugin_basename( $this->pluginFile ) ) {
								$links[] = " <a class='edit coption' href='" . esc_url( admin_url( 'admin-post.php' ) . '?action=?page=bixxs-events_fupc' ) . "'>Update Check</a>";
							}
							return $links;
						}, 10, 2 );
